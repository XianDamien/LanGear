...
- `GET /api/v1/study/session`：获取当前学习 session；返回 `server_time`、`session_date`、`scope`、`quota`、`summary` 与按 `learning/relearning -> review -> new` 排序的 `cards[]`；每张卡返回对外 `card_state`、`is_new_card`、`due_at`，其中 `new` 为派生态。
- `GET /api/v1/decks/tree`：获取 source-unit-lesson 树；lesson 节点至少返回 `total_cards`、`completed_cards`、`due_cards`、`new_cards`。
- `GET /api/v1/decks/{lesson_id}/cards`：获取 lesson 内卡片；每张卡至少返回 `card_state`、`is_new_card`、`due_at`、`last_review_at`。
