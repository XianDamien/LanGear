...
- `GET /api/v1/study/submissions/{id}`：轮询获取处理结果（processing/completed/failed）；完成态至少返回 `transcription.text`、兼容字段 `transcription.timestamps`、`feedback.suggestions[]`、`feedback.issues[]`、`oss_audio_path`。
- `POST /api/v1/study/submissions/{id}/rating`：提交评分，用于 FSRS 更新与学习统计；前端可提交 `1|2|3|4`，后端按 `again/hard/good/easy` 口径落库，并返回 FSRS 更新结果。
...
`review_log` 与 `user_card_srs` 同属 `DATABASE_URL` 指向的运行时数据库；二者是结构化结果与学习状态的真源，不存放在 OSS，也不以 `backend/datasets/` 为准。
