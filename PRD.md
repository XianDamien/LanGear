...
- `GET /api/v1/study/session`：获取当前学习 session；返回 `server_time`、`session_date`、`scope`、`quota`、`summary` 与按“FSRS 学习中/复习中卡片优先，再补充初始卡桶”的顺序返回的 `cards[]`；当请求带 `lesson_id` 时，还需要把该 lesson 中“已复习但尚未到期”的卡追加到队列尾部，保证刷新后课内视图仍能看到刚复习过的卡。每张卡返回 `card_state`、`is_new_card`、`due_at`、`last_review_at`，其中 `card_state` 只允许 `learning/review/relearning`。
