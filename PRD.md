...
> 快速沟通与阶段基准版本请优先查看：`PRD_BASELINE.md`（更新日期：2026-03-20）。
...
- 在卡片级别获得稳定、可回放对照的 AI 反馈（第一阶段聚焦 `issues[]` 的可选时间戳定位）。
- 在课文级别获得总结性复盘与下一步建议。
- 通过 FSRS 调度形成长期复习节奏。
...
### 2.3 AI 与云服务
- **ASR（语音转文本）**：阿里云 DashScope - qwen3-asr-flash 模型，通过 `dashscope.audio.asr.Recognition` API 调用。输入 OSS 签名 URL，返回完整转写文本，并在可用时附带词级时间戳（word-level timestamps）；第一阶段前端不依赖逐词时间戳跳转，展示文本以实时转写最终文本为准。
...
2) **停止录音与实时转写收口**：
   - 用户停止录音后，只结束本地录音与实时转写会话，不立即上传。
   - 只有当实时转写会话已产出最终文本时，才允许进入翻面提交阶段。
3) **翻面触发上传与排队（强制）**：
   - 用户点击翻面时，前端获取 STS 凭证并上传最后一次有效录音到 OSS（显示上传进度）。
   - 只有上传成功且 realtime session ready 时，才创建训练 submission（ASR → AI 评测）并开始轮询处理结果。
   - 若上传失败或 realtime session 未 ready：该卡片不得进入结果完成态，必须提示重试，但不阻塞继续切卡。
4) **背面复盘**：展示原音频 vs 用户音频、原文 vs 转写（普通文本展示）、AI 单句反馈；第一阶段重点支持 `issues[]` 的可选时间戳回听，不要求转录文本逐词点击跳转。
5) **评分（与 AI 反馈解耦）**：
   - 用户在前端选择 FSRS 整数评分 `1/2/3/4`，后端统一映射为 `again/hard/good/easy`。
   - **评分不会影响 AI 反馈生成结果**；评分仅用于 FSRS 调度与学习统计。
   - 评分作为单独步骤提交；提交成功后返回 FSRS 更新结果（至少包含 `state`、`difficulty`、`stability`、`due_at`），并进入下一张。
6) **下一张/课级总结**：卡片完成后进入下一张；完成整课后生成课级总结。

### 4.4 异步与轮询
- submission 创建后，前端轮询结果（processing/completed/failed）。
- 轮询超时 30 秒：提示“处理中，稍后在历史记录查看”，不阻塞继续学习。
- 处理失败不阻塞继续切卡，但该卡片不得进入结果完成态；用户需通过任务入口返回重试。
...
- Study Session：正反面训练流程、录音、翻面触发 OSS 上传、异步提交、轮询结果、评分；当前阶段由顶部任务导航承载跨卡片任务状态。
...
- ASR Service：调用阿里云 DashScope qwen3-asr-flash 模型完成用户录音转写，输入 OSS 签名 URL，返回 transcription，并在可用时附带 word-level timestamps。
...
- `GET /api/v1/study/session`：获取当前学习 session；返回 `server_time`、`session_date`、`scope`、`quota`、`summary` 与按 `learning/relearning -> review -> new` 排序的 `cards[]`。
- `POST /api/v1/study/submissions`：翻面后提交训练（携带 `oss_audio_path` 与 `realtime_session_id`），返回 `submission_id` 并进入异步处理队列。
- `GET /api/v1/study/submissions/{id}`：轮询获取处理结果（processing/completed/failed）；完成态重点返回 `transcription.text`、`feedback.suggestions[]`、`feedback.issues[]`、`oss_audio_path`，不把逐词时间戳跳转作为 V1 首批必达。
- `POST /api/v1/study/submissions/{id}/rating`：提交评分，用于 FSRS 更新与学习统计；前端可提交 `1|2|3|4`，后端按 `again/hard/good/easy` 口径落库。
...
### 7.1 AI 服务选型
- **ASR 转写**：阿里云 DashScope qwen3-asr-flash 模型。后端通过 `dashscope.audio.asr.Recognition` API 传入 OSS 签名 URL（`file_urls` 参数），在可用时可获取词级时间戳；第一阶段联调只要求稳定返回转写文本，不要求前端依赖逐词时间戳跳转。
...
### 7.2 单句反馈 JSON 结构
单句反馈必须包含转写文本与发音/完整度/流畅度三大核心指标；`transcription.timestamps` 作为兼容字段保留，第一阶段重点是 `feedback.issues[]` 的可选时间戳定位。固定结构化字段如下：
```json
{
  "transcription": {
...
**说明**：
- `transcription.timestamps`：兼容字段；可用于后续增强的逐词交互，但不是第一阶段必需字段。
- `feedback.suggestions`：建议数组（最多 3 条），允许带 `target_word` 和 `timestamp`，但第一阶段不要求基于 suggestions 做主交互。
- `feedback.issues`：问题数组（最多 5 条），每项包含 `problem` 与可选 `timestamp`；第一阶段重点要求前端消费 `issues[]`，`timestamp` 可为 `null`。
...
3) 单句反馈包含 `issues[]`，且 `issues[].timestamp` 可选支持跳转回听；AI 反馈无数值总分，也不要求逐词转录点击跳转。
