...
- `GET /api/v1/study/session`：获取当前学习 session；返回 `server_time`、`session_date`、`scope`、`quota`、`summary` 与按“FSRS 学习中/复习中卡片优先，再补充初始卡桶”的顺序返回的 `cards[]`；卡片自身 `card_state` 仅返回 `learning/review/relearning`，是否属于“新卡”通过 `is_new_card` 派生。
