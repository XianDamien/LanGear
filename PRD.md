...
2) **翻面触发上传与排队（强制）**：
   - 每次录音结束后只缓存到浏览器，不上传。
   - 用户点击翻面时，才上传“最后一次缓存录音”到 OSS，并立即创建 submission 开始排队 ASR + AI 分析。
   - 若上传失败：不允许翻面，必须提示重试。
3) **背面复盘**：展示原音频 vs 用户音频、原文 vs 转写（含词级时间戳）、AI 单句反馈与可跳转建议。
4) **评分**：用户选择 again/hard/good/easy，系统持久化评分并驱动 FSRS 更新。
5) **下一张/课级总结**：卡片完成后进入下一张；完成整课后生成课级总结。
...
本总纲只描述关键链路，具体字段与响应结构以 `backend/PRD_MVP_BACKEND.md` 为准。
