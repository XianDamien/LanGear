...
### 2.5 时间与业务时区
- 系统采用“单实例单业务时区”模型，不在 `users` 表中存储用户独立 timezone。
- V1 固定使用北京时间 `Asia/Shanghai`，不提供运行时切换业务时区能力。
- 数据库中的业务时间统一按“北京时间本地 naive datetime”存储；历史旧数据从“UTC-naive”升级时，需在 Alembic 迁移中一次性平移到北京时间。
- API 对外时间统一返回带 `+08:00` 偏移的 ISO 8601 字符串；前端不得假定 `Z`。
...
4) **背面复盘**：顶部展示左右对照的双播放器（左侧原音频、右侧用户音频，控件样式统一）；中部展示单框文本对照区（原文 + Gemini 展示转写，译文默认隐藏并在同框内展开），原文与转写按词级机械 diff 呈现；底部展示 AI 单句反馈与笔记。
   - 回听定位仅来自问题点时间戳，且时间戳跳转只作用于用户音频，不对原音频提供 seek。
...
- `GET /health`、Study/Deck/Dashboard/Submission 相关接口中的时间字段统一按北京时间序列化，必须输出带 `+08:00` 偏移的 ISO 8601。

## 6. 数据库设计（6 张表）
数据库采用 SQLite 单库。教材层级使用单表树形表达，卡片作为最小训练单元。字段定义如下。

> 时间存储总原则：除非字段明确另有说明，业务相关 datetime 列统一存储为“北京时间本地 naive datetime”；对外 API 序列化时再补上 `+08:00` 偏移。
...
- review_datetime：评分发生时间（业务时区本地 naive datetime）。
...
- updated_at：更新时间。
- V1 不提供 `app_timezone` 运行时配置项；全链路固定使用北京时间。
